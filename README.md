...
api/users:  These API's are guarded by @token_required, test these using Postman, first you must authenticate http://127.0.0.1:8082/api/users/authenticate using raw body JSON {"uid": "toby", "password": "123toby"} which will obtain a Cookie. Then you can use the API http://127.0.0.1:8082/api/users

api/players: These API's are unguarded. The GET method can be run in the browser http://127.0.0.1:8082/api/players/.   PUT, POST, DELETE methods are supported and can be tested in POSTMAN. 
